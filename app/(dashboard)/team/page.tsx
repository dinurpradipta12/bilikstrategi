...
export default function TeamWorkloadPage() {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      {activeTab === 'chart' && (
        <div className="space-y-6 animate-fade-in">
          <div className="p-8 bg-[#FFFFFF] border border-[#E8E8EC] rounded-2xl shadow-2xs text-center relative overflow-hidden min-h-[420px] flex flex-col items-center justify-center space-y-8 bg-grid-pattern">
            {/* Top Root Manager */}
            <div className="flex flex-col items-center space-y-2 relative z-10">
              <div className="px-6 py-3 bg-[#24324A] text-[#FFFFFF] rounded-2xl shadow-lg border border-[#24324A] flex items-center gap-3">
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img src={members[0]?.avatar_url || 'https://ui-avatars.com/api/?name=Dinur+Pradipta'} alt="Manager" className="w-8 h-8 rounded-full border border-white" />
                <div className="text-left">
                  <h4 className="text-xs font-bold">{members[0]?.full_name || 'Dinur Pradipta'}</h4>
                  <span className="text-[10px] text-[#EEF2F7]">Workspace Owner / Lead Manager</span>
                </div>
              </div>
              <span className="px-3 py-1 bg-[#EEF2F7] text-[#24324A] text-[10px] font-bold rounded-full border border-[#E8E8EC]">
                Direct Reports ({members.length - 1 || 5})
              </span>
            </div>

            {/* Connecting Vertical Line */}
            <div className="w-0.5 h-8 bg-[#24324A]" />

            {/* Subordinate Nodes Grid */}
            <div className="grid grid-cols-2 md:grid-cols-5 gap-4 relative z-10 w-full max-w-4xl">
              {members.slice(1).map((m) => (
                <div key={m.id} className="p-3 bg-[#F7F7F8] border border-[#E8E8EC] rounded-xl text-center space-y-2 hover:border-[#24324A] transition-all shadow-2xs">
                  {/* eslint-disable-next-line @next/next/no-img-element */}
                  <img src={m.avatar_url} alt={m.full_name} className="w-10 h-10 rounded-full mx-auto border border-[#E8E8EC]" />
                  <div>
                    <h5 className="text-xs font-bold text-[#24324A] truncate">{m.full_name}</h5>
                    <span className="text-[10px] text-[#737680] capitalize">{m.role}</span>
                  </div>
                </div>
              ))}
            </div>

            {/* Canvas Zoom Controls Footer */}
            <div className="absolute bottom-4 right-4 flex items-center gap-2 bg-[#FFFFFF] border border-[#E8E8EC] px-3 py-1.5 rounded-xl shadow-2xs text-xs font-bold text-[#24324A]">
              <button className="hover:text-[#F26B5E] cursor-pointer">-</button>
              <span>80%</span>
              <button className="hover:text-[#F26B5E] cursor-pointer">+</button>
              <span className="text-[#E8E8EC]">|</span>
              <span className="text-[10px] text-[#737680]">Expand all ({members.length})</span>
            </div>
          </div>
        </div>
      )}
